...
impl PyO3TriTile {
    ...
    fn corner_ids<'py>(&self, py: Python<'py>) -> &'py PyArray2<i64> {
        let corner_ids: numpy::ndarray::Array2<i64> = self._tile.corner_ids();
        corner_ids.into_pyarray(py)
    }

    fn corners<'py>(&self, py: Python<'py>) -> &'py PyArray2<f64> {
        let corners: numpy::ndarray::Array2<f64> = self._tile.corners();
        corners.into_pyarray(py)
    }

    fn indices<'py>(&self, py: Python<'py>) -> &'py PyArray3<i64> {
        let ids: numpy::ndarray::Array3<i64> = self._tile.indices();
        ids.into_pyarray(py)
    }
...
impl PyO3RectTile {
    ...
    fn corner_ids<'py>(&self, py: Python<'py>) -> &'py PyArray2<i64> {
        let corner_ids: ndarray::Array2<i64> = self._tile.corner_ids();
        corner_ids.into_pyarray(py)
    }

    fn corners<'py>(&self, py: Python<'py>) -> &'py PyArray2<f64> {
        let corners: ndarray::Array2<f64> = self._tile.corners();
        corners.into_pyarray(py)
    }

    fn indices<'py>(&self, py: Python<'py>) -> &'py PyArray3<i64> {
        let ids: ndarray::Array3<i64> = self._tile.indices();
        ids.into_pyarray(py)
    }
