...
# nocheckin, This is a clone of rotated_grids.py and should not be merged into main
...
grid = TriGrid(size=1, rotation=-20, orientation="pointy")
grid_flat = TriGrid(size=1, rotation=-20, orientation="flat")
# breakpoint()

ids_x = ids_y = numpy.arange(10) - 5
...
# colors = point_ids.index[:, 1]

# Plot points in cell
plt.scatter(*points.T, s=3, c=colors, cmap="prism")
# plt.scatter(*points.T, s=3, c=colors, cmap="viridis")
for id in ids:
    center = grid.centroid(id)
    plt.text(center[0] - 0.2, center[1], id, size=10)
plot_polygons(geometries.geoms, colors="black", fill=False, linewidth=2)
plt.show()
...
def plot_rotated_grid(rotation, ax):
    grid = TriGrid(size=1, rotation=rotation, orientation="pointy")
    geometries = grid.to_shapely(ids, as_multipolygon=True)
