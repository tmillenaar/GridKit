...
class BoundedGrid(metaclass=BoundedGridMeta):
    ...
    @classmethod
    def interp_from_points(cls, points, values, grid, method="linear", nodata_value=numpy.nan):
        """Interpolate the cells containing nodata, if they are inside the convex hull of cells that do contain data.
        ...
        grid: :class:`~gridkit.base_grid.BaseGrid`
            The grid that will be used to interpolate onto.
        method: :class:`str`
            The interpolation method to be used. Options are ("nearest", "linear", "cubic"). Default: "linear".
        ...
        return cls(data=interp_values, bounds=aligned_bounds, crs=grid.crs, nodata_value=nodata_value)
