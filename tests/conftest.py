import pytest
import numpy
from gridkit import rect_grid, hex_grid
...
def basic_bounded_pointy_grid():
    ...
    return hex_grid.BoundedHexGrid(data, bounds=bounds, nodata_value = -9999, shape="pointy")

@pytest.fixture(scope="function")
def basic_bounded_flat_grid():
    data = numpy.arange(2*3).reshape(2,3)
    bounds = (-1.299038105676658, -1.5, 1.299038105676658, 3.0)
    return hex_grid.BoundedHexGrid(data, bounds=bounds, nodata_value = -9999, shape="flat")
